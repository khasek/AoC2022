...
# between stacks (SEVERAL CRATES CAN NOW BE MOVED AT ONCE). 
...
def MoveCrates(num_moved, from_stack, to_stack):
    ...
    while iter < num_moved:
        temp_stack.append(crates[from_stack].pop())
        iter += 1

    iter = 0
    while iter < num_moved:
        crates[to_stack].append(temp_stack.pop())
        iter += 1
